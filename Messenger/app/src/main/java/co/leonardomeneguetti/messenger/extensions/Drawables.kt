package co.leonardomeneguetti.messenger.extensions
...
fun Drawable.toCircle(resources: Resources, dp: Int): Drawable {
    ...
    val width: Int
    if (aspectRatio < 1) {
        width = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, dp.toFloat(), resources.displayMetrics).toInt()
        height = (width / aspectRatio).toInt()
    } else {
        height = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, dp.toFloat(), resources.displayMetrics).toInt()
